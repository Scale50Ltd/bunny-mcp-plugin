...
capture → cheap PREFILTER (structural hard-kills)
        ...
        → human finalizes the required doc → conveyor advances 3 → 4
```
...
- **`greenlight_bunny`** — **ask the user first.** The human gate that advances a **parked** (`build`/`research`) bunny from Phase 2 to **Phase 3 (EVIDENCE)**. Pass `overrideKill: true` only if the user explicitly wants to push a `kill`-verdict bunny through anyway. Never greenlight to self-advance your own work.
...
3. **You never bless your own work and never self-advance.** Every gate is the human's: `screen_bunny` (enter Phase 2), `greenlight_bunny` (Phase 2 → 3), and `finalize_document → final` (the 3 → 4 conveyor) are all allowed **only on the user's explicit command — never autonomously, and never to advance your own research**. The token can reach these; the discipline that they're human-decided is yours.
